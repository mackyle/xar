...
void
xar_compare (xar_toc_t one, xar_toc_t two)
{
...
void
xar_diff (char *file1, char *file2)
{
  ...
  for (t1 = xar_toc_get_first_element (one); t1;
       t1 = xar_toc_get_next_element (t1))
    {
  ...
  for (t1 = xar_toc_get_first_element (two); t1;
       t2 = t1, t1 = xar_toc_get_next_element (t1), free (t2))
    {
      char *path = xar_toc_get_path (t1);
